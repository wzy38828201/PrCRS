...
#这里有w的原因是这里要测试
with torch.no_grad():
    testset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\9个测试\data193.npy')#'./data2/data2_test多.npy'
...
df4[1] = df3[1]
df4[2] = df3[3]
df4.to_excel(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\9个测试\W总.xlsx',index = None)


testset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\9个测试\data9.npy')#'./data2/data2_test多.npy'
test_loader = DataLoader(testset, num_workers=0,
                      shuffle=False, batch_size=1,
                      pin_memory=False, drop_last=True,
                      collate_fn=collate_fn)
...
gai[1] = shuchu
gai.to_excel(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\9个测试\W9总.xlsx',index = None)