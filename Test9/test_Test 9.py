...
if np.array(xx).shape[1]==43:
    ckpt_path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\12个测试\模型\全部,0.0003,20\91.pt'
...
#这里有w的原因是这里要测试
with torch.no_grad():
    testset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\12个测试\data190.npy')#'./data2/data2_test多.npy'
...
#    pd.DataFrame(wz).to_excel(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\12个测试\W总.xlsx',index = False)
...
df4[1] = df3[1]
df4[2] = df3[3]
df4.to_excel(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\12个测试\W总.xlsx',index = None)


testset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\12个测试\data12.npy')#'./data2/data2_test多.npy'
test_loader = DataLoader(testset, num_workers=0,
                      shuffle=False, batch_size=1,
                      pin_memory=False, drop_last=True,
                      collate_fn=collate_fn)
...
gai[1] = shuchu
gai.to_excel(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\12个测试\W12总.xlsx',index = None)