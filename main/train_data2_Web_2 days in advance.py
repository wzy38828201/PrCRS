...
from sklearn.metrics import f1_score, recall_score, accuracy_score, precision_score, confusion_matrix,roc_curve, auc
import warnings
...
# 对数据集进行验证，返回准确率和loss
def validate(model, data_loader, criterion, thresh=0.1):#0.3
    model.eval()
    y_labels = []
    y_preds = []
    losses = []
...
#0.0003,0.0002,0.0001,0.00008,0.00006,0.00005,0.00004,0.00003,
lr = [0.00035, 0.0003,0.00025,0.0002,0.00015,0.0001,]#0.00005,0.00004,0.00003,0.00002,0.00001]
bs = [16,18,20,22,24]
# lr = [0.0001]
# bs = [24]
def train():
    ...
    for ilr in lr:
        for jbs in bs:
            path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\output迁移提前2天'
            foldername = path + "\\"  + "除去细胞因子," + str(ilr) + "," + str(jbs)
            # print(pwd)
            # 文件路径
            ...
            output_dir = foldername#'./output迁移提前2天'#s2
            ...
            data1_ckpt = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\outputs1\新\全部,0.00015,22\62.pt'
            ...
            #seed123，27个特征，0.3：“2e-4，16，第63，64”“5e-4，16，第51”“1e-3，20，第42，88”“1e-4，20，第35,95”
            optim = torch.optim.Adam(model.parameters(), lr=ilr)#3e-4,这个参数适应最好
            ...
            devset = Dataset('./data提前2天/data2_valid多2.npy')
            dev_loader = DataLoader(devset, num_workers=0,
                                      shuffle=False, batch_size=1,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            testset = Dataset('./data提前2天/data2_test多2.npy')
                ...
                if train_avg_re >= 0.8:
                    if dev_avg_re >= 0.6:
                        if dev_avg_pr >= 0.2:
                            if test_avg_re >= 0.6:
                                if test_avg_pr >= 0.2:
                                    po = train_avg_re+dev_avg_re+test_avg_re#+train_avg_pr+dev_avg_pr+test_avg_pr
                                    dic[i] = po
            ...
            import operator
            if len(dic.keys())==0:
                print('当前特征不足以完成高准确的预测，建议增加特征后再次预测')
            else:
                max_key = max(dic.items(), key=operator.itemgetter(1))[0]
                print('最好的模型：',max_key,'对应的训练验证测试召回率：',dic_tr[max_key], dic_de[max_key], dic_te[max_key],
                      '对应的训练验证测试精准率：',dic2_tr[max_key], dic2_de[max_key], dic2_te[max_key])
                
                dicc[ilr, jbs, max_key] = dic_tr[max_key] + dic_de[max_key] + dic_te[max_key]# + dic2_tr[max_key] + dic2_de[
                #max_key] + dic2_te[max_key]
                max_keyc = max(dicc.items(), key=operator.itemgetter(1))[0]
                file_handle = open(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\output迁移提前2天\提前两天.txt', mode='a+')
                file_handle.write('除去细胞因子最好的模型：'+str(max_keyc)+str(dicc[max_keyc])+'\n')

    if len(dicc.keys())==0:
        print('当前特征不足以完成高准确的预测，建议增加特征后再次预测')
    else:
        max_keyc = max(dicc.items(), key=operator.itemgetter(1))[0]
        print('最好的模型：',max_keyc,dicc[max_keyc])
    model.zero_grad()
    optim.zero_grad()
