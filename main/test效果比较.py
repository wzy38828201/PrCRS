...
def main():
    ...
    if np.array(xx).shape[1]==43:
        #ckpt_path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\output无迁移\全部,0.0003,18\15.pt'
        ckpt_path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\output有迁移\全部,0.0003,20\30.pt'
        #ckpt_path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\outputs1\新\全部,0.00015,22\62.pt'
    ...
    #这里有w的原因是这里要测试
    with torch.no_grad():
        testset = Dataset('./data2/data2_test多.npy')#'./data2/data2_test多.npy'
        ...
        valset = Dataset('./data2/data2_valid多.npy')
...
#
##        pd.DataFrame(y_labels).to_csv(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\无迁移\labels.csv', index=False, header=False)
##        pd.DataFrame(y_preds).to_csv(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\无迁移\prediction.csv', index=False, header=False)
##        pd.DataFrame(logits).to_csv(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\是否迁移效果比较\无迁移\logits.csv', index=False, header=False)
#
#
