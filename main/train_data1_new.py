...
def train():
    ...
    for ilr in lr:
        for jbs in bs:
            path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\outputs1\新'
            foldername = path + "\\" + "全部," + str(ilr) + "," + str(jbs)
            # print(pwd)
            # 文件路径
            ...
            model = tranMedical(num_class=3).cuda()#x新冠肺炎是3分类
            model.zero_grad()
            ...
            collate_fn = Collate()
            batch_size =jbs#20
            trainset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\data3\data1_train.npy')
            train_loader = DataLoader(trainset, num_workers=0,
                                      shuffle=False, batch_size=batch_size,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            devset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\data3\data1_valid.npy')
            dev_loader = DataLoader(devset, num_workers=0,
                                      shuffle=False, batch_size=1,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            testset = Dataset(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\data3\data1_test.npy')
            ...
            if len(dic.keys())==0:
                ...
                dicc[ilr,jbs,max_key] = dic2_te[max_key]+dic_te[max_key]#+dic2_tr[max_key]+dic2_de[max_key]+dic2_te[max_key]
                max_keyc = max(dicc.items(), key=operator.itemgetter(1))[0]
                file_handle = open(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\outputs1\新\当天.txt', mode='a+')
                file_handle.write('全部最好的模型：'+str(max_keyc)+str(dicc[max_keyc])+'\n')
