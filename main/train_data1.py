...
def train():
    ...
    for ilr in lr:
        for jbs in bs:
            ...
            model = tranMedical(num_class=3).cuda()#COVID-19 is classified into three categories
            model.zero_grad()
            ...
            collate_fn = Collate()
            batch_size =jbs#20
            trainset = Dataset('main/data/data1_train.npy')
            train_loader = DataLoader(trainset, num_workers=0,
                                      shuffle=False, batch_size=batch_size,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            devset = Dataset('main/data/data1_valid.npy')
            dev_loader = DataLoader(devset, num_workers=0,
                                      shuffle=False, batch_size=1,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            testset = Dataset('main/data/data1_test.npy')
            ...
            if len(dic.keys())==0:
                ...
                file_handle = open('main/outputs1/1_day.txt', mode='a+')
