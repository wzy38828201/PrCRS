...
lr = [0.0003,0.00025,0.0002,0.00015]#0.00005,0.00004,0.00003,0.00002,0.00001]
bs = [18,20,22,24]
# lr = [0.0001]
# bs = [24]
def train():
    ...
    for ilr in lr:
        for jbs in bs:
            path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\output迁移提前3天'
            foldername = path + "\\"  + "生化血常规," + str(ilr) + "," + str(jbs)
            # print(pwd)
            # 文件路径
            ...
            collate_fn = Collate()
            batch_size =jbs #20
            trainset = Dataset('./data提前3天/data2_train多2.npy')
            train_loader = DataLoader(trainset, num_workers=0,
                                      shuffle=False, batch_size=batch_size,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            devset = Dataset('./data提前3天/data2_valid多2.npy')
            ...
            if len(dic.keys())==0:
                ...
                file_handle = open(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\output迁移提前3天\提前3天.txt', mode='a+')
                file_handle.write('生化血常规最好的模型：'+str(max_keyc)+str(dicc[max_keyc])+'\n')
