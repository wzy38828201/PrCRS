...
def train():
    ...
    optim = torch.optim.Adam(model.parameters(), lr=2e-4)#1e-5
    criterion = torch.nn.CrossEntropyLoss()

    # step2: def 训练dataloader，验证dataloader以及测试dataloader
    collate_fn = Collate()
    batch_size = 32#16
    trainset = Dataset('./data提前2天/data2_train.npy')
    train_loader = DataLoader(trainset, num_workers=0,
                              shuffle=False, batch_size=batch_size,
                              pin_memory=False, drop_last=True,
                              collate_fn=collate_fn)

    devset = Dataset('./data提前2天/data2_valid.npy')
    dev_loader = DataLoader(devset, num_workers=0,
                              shuffle=False, batch_size=1,
                              pin_memory=False, drop_last=True,
                              collate_fn=collate_fn)

    testset = Dataset('./data提前2天/data2_test.npy')
