...
xx = [['D-二聚体', '降钙素原', 'B型尿钠肽', 'α羟丁酸脱氢酶', '前白蛋白', '原幼细胞',  # 和下面的凝血项一起为2
       ...
       '钠', '钾', '氯', '钙', '尿酸', '葡萄糖', '甘油三酯', 'γ-谷氨酰转肽酶', '白蛋白', '谷丙转氨酶', '谷草转氨酶', '碱性磷酸酶', '乳酸脱氢酶', '肌酐', 'C反应蛋白',
       '铁蛋白',  # 生化项2
...
if np.array(xx).shape[1] == 43:
    ckpt_path = r'\main\Model comparison\PrCRS\all.pt'
...
# 这里有w的原因是这里要测试
with torch.no_grad():
    testset = Dataset(r'\main\Test5\data202.npy')  # './data2/data2_test多.npy'
...
df4[1] = df3[1]
df4[2] = df3[3]
df4.to_excel(r'\main\Test5\W总.xlsx', index=None)

testset = Dataset(r'\main\Test5\data5.npy')  # './data2/data2_test多.npy'
test_loader = DataLoader(testset, num_workers=0,
                         shuffle=False, batch_size=1,
                         pin_memory=False, drop_last=True,
                         collate_fn=collate_fn)
...
gai[1] = shuchu
gai.to_excel(r'\main\Test5\W5总.xlsx', index=None)