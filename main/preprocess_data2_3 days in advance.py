...
#xlsx_file2 = './data2/data2.xlsx'
xlsx_file2 = r'G:\CRS预测\程序分析\分类预测\决策树可视化\xgb_visualization\批量生成\病人体质易不易发生CRS预测\易发体质预测全部信息新新迁移用.xlsx'
...
for jj1,ii1 in enumerate(b):
    ...
    if jj1==0:
        cc = di
    if jj1!=0:
        cc = pd.concat([cc,di],ignore_index=True)

ccc = pd.DataFrame()
a_b = cc
for jj2,ii2 in enumerate(b):
    aa = a_b[a_b['姓名'] == ii2]
    di = pd.DataFrame()
    crs = list(aa['CRS'])
    del(crs[0])
    ...
    if jj2==0:
        ccc = di
    if jj2!=0:
        ccc = pd.concat([ccc,di],ignore_index=True)
...
#                '红细胞', '血红蛋白', '白细胞','中性粒细胞百分比', '中性粒细胞计数', '淋巴细胞百分比', '淋巴细胞计数','血小板', '单核细胞百分比', '单核细胞计数', #血常规3
#                '钠', '钾', '氯', '钙', '尿酸', '葡萄糖', '甘油三酯', 'γ-谷氨酰转肽酶','白蛋白', '谷丙转氨酶', '谷草转氨酶', '碱性磷酸酶', '乳酸脱氢酶', '肌酐', 'C反应蛋白', '铁蛋白',#生化项2
#                 'IL2', 'IL4', 'IL6', 'IL10', 'TNFα', 'IFNγ', 'IL17A'] #
#common_keys = ['红细胞', '血红蛋白', '白细胞','中性粒细胞百分比', '中性粒细胞计数', '淋巴细胞百分比', '淋巴细胞计数','血小板', '单核细胞百分比', '单核细胞计数']
#common_keys = ['钠', '钾', '氯', '钙', '尿酸', '葡萄糖', '甘油三酯', 'γ-谷氨酰转肽酶','白蛋白', '谷丙转氨酶', '谷草转氨酶', '碱性磷酸酶', '乳酸脱氢酶', '肌酐', 'C反应蛋白', '铁蛋白']#生化项2
...
np.save(os.path.join(savedir, 'data2_train多2.npy'), np.array(train_data2_feats_label))
np.save(os.path.join(savedir, 'data2_valid多2.npy'), np.array(valid_data2_feats_label))
np.save(os.path.join(savedir, 'data2_test多2.npy'), np.array(test_data2_feats_label))
