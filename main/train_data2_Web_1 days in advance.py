...
def train():
    ...
    for ilr in lr:
        for jbs in bs:
            path = r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\output多'
            ...
            collate_fn = Collate()
            batch_size =jbs#20
            trainset = Dataset('./data2/data2_train多.npy')
            train_loader = DataLoader(trainset, num_workers=0,
                                      shuffle=False, batch_size=batch_size,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            devset = Dataset('./data2/data2_valid多.npy')
            dev_loader = DataLoader(devset, num_workers=0,
                                      shuffle=False, batch_size=1,
                                      pin_memory=False, drop_last=True,
                                      collate_fn=collate_fn)

            testset = Dataset('./data2/data2_test多.npy')
            ...
            if len(dic.keys())==0:
                ...
                dicc[ilr,jbs,max_key] = dic_tr[max_key]+dic_de[max_key]+dic_te[max_key]#+dic2_tr[max_key]+dic2_de[max_key]+dic2_te[max_key]
                max_keyc = max(dicc.items(), key=operator.itemgetter(1))[0]
                file_handle = open(r'G:\图神经网络编程\深度学习预测CRS\迁移学习\code2\output多\提前一天.txt', mode='a+')
